...
[~,n] = size(results);
for i=1:n
    fprintf('%d\t%s\tscore=%.2f\tdist=%.2f\thighest_score=%.2f\n',i,results(pos(i)).name,results(pos(i)).final_score,results(pos(i)).avg_dist,results(pos(i)).highest_score);
end
...
for i=1:n
    fprintf(fp,'%d\t%s\tscore=%.2f\tdist=%.2f\thighest_score=%.2f\n',i,results(pos(i)).name,results(pos(i)).final_score,results(pos(i)).avg_dist,results(pos(i)).highest_score);
end
fprintf(fp,'\n');
% fprintf(fp,'======================== test end ========================\n\n\n');
